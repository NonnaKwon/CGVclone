...
function TimeCard({time,time2}){
    ...
    const seat = ((start1,start2,seat)=>{
        start1 = moment(start1).format('HH:mm');
        start2 = moment(start2).format('HH:mm');
        if(seat > 0){ //seat 이 시간마다 있어야함.. ㄷㄷ
            return(
                <div className="reservation">
                    <div className="time">{start1}<div className="num">{seat}석</div></div>
                    <div className="time">{start2}<div className="num">{seat}석</div></div>
                </div>
            );
        }else{
            return(
    ...
    return(
        <div className="timecard_container">
            ...
            <div className="seat">2D&nbsp;&nbsp;&nbsp;&nbsp;<span className="s">|</span>&nbsp;&nbsp;&nbsp;&nbsp;
                ...
                {seat(time.start_time,time2.start_time,time.reservationable_seat_num)}
        </div>
    );
}
